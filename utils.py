...
def visualize_trajectory_2d(pose,path_name="imu trajectory",show_ori=False):
  '''
...
def visualize_trajectory_2d_2(pose,pose1,path_name="imu trajectory with no update",path_name1="imu trajectory with VSLAM",show_ori=False):
  '''
  function to visualize the trajectory in 2D
  Input:
      pose:   4*4*N matrix representing the camera pose, 
  ...
  n_pose1 = pose1.shape[2]
  ax.plot(pose1[0,3,:],pose1[1,3,:],'g-',label=path_name1)
  ax.scatter(pose1[0,3,0],pose1[1,3,0],marker='s',label="start")
  ax.scatter(pose1[0,3,-1],pose1[1,3,-1],marker='o',label="end")
  if show_ori:
      select_ori_index = list(range(0,n_pose1,int(n_pose1/50)))
      yaw_list = []
      for i in select_ori_index:
          _,_,yaw = mat2euler(pose1[:3,:3,i])
          yaw_list.append(yaw)
      dx = np.cos(yaw_list)
      dy = np.sin(yaw_list)
      dx,dy = [dx,dy]/np.sqrt(dx**2+dy**2)
      ax.quiver(pose1[0,3,select_ori_index],pose1[1,3,select_ori_index],dx,dy,\
          color="b",units="xy",width=2)
  ax.set_xlabel('x')
  ax.set_ylabel('y')
  ax.axis('equal')
